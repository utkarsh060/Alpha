...
option = st.sidebar.selectbox("Dashboard?" , ('Home-Page','Sentiment Analysis','Portfolio Analysis','Technical Analysis' ,'Forecast'),0)
...
if option == 'Home-Page':
    ...
    with row1_1:
        ...
        with Disclaimer:
            st.markdown("""The content of this web app is for educational purposes only.
        ...
        st.info('Please scroll through different dashboards using the navigation tab on the upper left corner.')


# In[7]:


stocksymbols = ["Select a Ticker","^NSEI","^NSEBANK",'^BSESN','ADANIENT.NS','ADANIPOWER.NS','ADANIGREEN.NS','ADANIPORTS.NS',
 'BHARTIARTL.NS', 'TATAMOTORS.NS','DABUR.NS', 'ICICIBANK.NS', 'GAIL.NS','MARUTI.NS', 'COALINDIA.NS','SHREECEM.NS','IOC.NS','HCLTECH.NS',
'BRITANNIA.NS','ASIANPAINT.NS', 'TECHM.NS','WIPRO.NS','EICHERMOT.NS','TCS.NS', 'BPCL.NS','LT.NS',
 'INDUSINDBK.NS','IRCTC.NS','IGL.NS',
 'INFY.NS',
 'HEROMOTOCO.NS',
 'TITAN.NS','TATAPOWER.NS','TATASTEEL.NS',
 'M&M.NS',
 'GRASIM.NS',
 'ONGC.NS',
 'SBILIFE.NS','SUNPHARMA.NS','SPICEJET.NS','SCI.NS',
 'BAJAJFINSV.NS',
 'RELIANCE.NS',
 'HINDALCO.NS',
 'HDFCLIFE.NS',
 ...
 'CIPLA.NS',
 'ITC.NS',
 'ULTRACEMCO.NS','UPL.NS',
 'DRREDDY.NS',
 'KOTAKBANK.NS',
 'DIVISLAB.NS',"IONEXCHANG.BO","PITTIENG.NS","DAAWAT.NS","SURYAROSNI.NS","UFLEX.NS","NACLIND.NS", "SOBHA.NS" ,"AKCAPIT.BO",'AXISBANK.NS',
 'TATASTEEL.NS',
 'SBIN.NS','PNB.NS',"INR=X"]
...
try:
    if option == 'Sentiment Analysis':
        ...
        if query != '' and query != '#':
            ...
            if noOfTweet != '' :
                noOfDays = st.text_input("Enter number of days you want to Scrape Twitter for",2)
                if noOfDays != '':
                    with st.spinner(f'Searching for and analyzing {query}, Please be patient, it might take a while...'):
                        ...
                        tweets=tweepy.Cursor(api.search,q=query,lang="en",since=date_since).items(int(noOfTweet))

                        for tweet in tweets:
                            tweets_list1.append([tweet.created_at,tweet.user.location,tweet.favorite_count,tweet.user.screen_name,tweet.text]) 
                        ...
                        tw_df = pd.DataFrame(tweets_list1, columns=['Date','Location','Likes', 'Username', 'Text'])
                        ...
                        # Print Statements
                        st.write("Since " + noOfDays + " days, there have been", count ,  "tweets on " + query, end='\n*')
                        ...
                        st.write("**Compound Sentiment:**", '%.2f' % avg_compound, end='\n')


                        #Creating PieCart
                        labels = ['Positive ['+str(avg_pos)+'%]' , 'Neutral ['+str(avg_neu)+'%]','Negative ['+str(avg_neg)+'%]']
                        ...
                        ax.legend(labels)
                        ax.set_title("Sentiment Analysis Result for keyword= "+query+"" )
                        ax.axis('equal')
                        st.pyplot(fig)
        ...
        if company_name != '':
            with st.spinner(f'Searching for and analyzing {company_name}, Please be patient, it might take a while...'):
                ...
                try:
                    ...
                    st.write("**Compound Sentiment:**:", '%.2f' % N_avg_compound, end='\n')
                    ...
                    st.write('Seems like there is some problem in fetching  the data, Please try after some time or try with a different ticker.' )

except Exception as e:
    st.write("exception occurred:" + str(e))
    st.write('Looks like, There is some error in retrieving the data, Please try again!')
...
try:
    if option == 'Portfolio Analysis':
        ...
        portfolio = st.multiselect("Please select Stocks to create a portfolio", stocksymbols,default=["TATAMOTORS.NS"])
        try:
            df = getMyPortfolio(portfolio)
            df_main = df['Adj Close']
            numAssets = len(portfolio)
            portfolio_amount = st.text_input("Enter the amount you want to invest",int(50000))
            with st.spinner(f"You have {numAssets} assets in your porfolio, Do you want to add more? If not then click on Analyze." ):
                pass
        except RemoteDataError:
            pass

        if st.button("Analyze"):
            with st.spinner(f'Searching for and analyzing your portfolio, Please be patient, it might take a while...'):
                st.write(df_main)
                ...
                fig2 = plt.figure()
                sb.heatmap(daily_simple_return.corr(),xticklabels=daily_simple_return.corr().columns, yticklabels=daily_simple_return.corr().columns,
                cmap='cubehelix', annot=True, linewidth=0.5)
                st.subheader('Correlation between daily simple returns of stocks in your portfolio')
                st.pyplot(fig2)
                ...
                cleaned_weights = ef.clean_weights() #to clean the raw weights, %setting any weights whose absolute value are below the *cutoff* to 0%, and rounding the rest 
                ...
                if portfolio_amount != '' :
                    ...
                    portfolio_df = pd.DataFrame(columns =['Company name' , 'Ticker' , 'Number of stocks to buy'])
                    ...
                    st.subheader('Number of stocks to buy with the amount of ₨ ' + str(portfolio_amount))
...
try:
    if option == 'Technical Analysis':
        ...
        if selected_stocks == "Select a Ticker":
            ...
            ax.plot(data['Adj Close'] , label = selected_stocks ,linewidth=0.5, color='blue', alpha = 0.9)
            ...
            ax.set_title(selected_stocks + " Price History with buy and sell signals",fontsize=10, backgroundcolor='blue', color='white')
            ...
            def heikin_ashi(df):
                ...
                heikin_ashi_df['HA_close'] = (df['Open'] + df['High'] + df['Low'] + df['Adj Close']) / 4

                for i in range(len(df)):
                    if i == 0:
                        heikin_ashi_df.iat[0, 0] = (df['Open'].iloc[0] + df['Adj Close'].iloc[0])/2
                    else:
            ...
            def Heiken_Strategy(data,data1):
                ...
                for i in range(len(data)):
                    if data['HA_close'][i] > data['HA_open'][i] and data['HA_close'][i - 1] < data['HA_open'][i - 1] and data['HA_close'][i - 2] < data['HA_open'][i - 2] :
                        if flag == False:
                    ...
                    elif data['HA_close'][i] < data['HA_open'][i] and data['HA_close'][i - 1] < data['HA_open'][i - 1] and data['HA_close'][i - 2] > data['HA_open'][i - 2] :
                        if flag == True:
                            buy_price.append(np.nan)
            ...
            ax.scatter(HA.index , HA['sell here'] , label = 'Sell' , marker = 'v', color = 'red' ,alpha =1)
            ax.set_title(selected_stocks + " price History with buy and sell signals as per Heiken Ashi strategy",fontsize=10, backgroundcolor='blue', color='white')
            ax.set_xlabel(start_date + ' - ' + end_date , fontsize=18)
            ax.set_ylabel('Close Price INR (₨)' , fontsize=18)
            legend = ax.legend()
            ax.grid()
            ...
            ax.scatter(data.index , data['DEMA_Buy_Signal_price'] , label = 'Buy' , marker = '^', color = 'green',alpha =1 )
            ax.scatter(data.index , data['DEMA_Sell_Signal_price'] , label = 'Sell' , marker = 'v', color = 'red',alpha =1 )
            ax.set_title(selected_stocks + " Price History with buy and sell signals as per Double Exponential Moving Average",fontsize=10, backgroundcolor='blue', color='white')
            ax.set_xlabel(start_date + ' - ' + end_date, fontsize=18)
            ax.set_ylabel('Close Price INR (₨)' , fontsize=18)
            legend = ax.legend()
            ax.grid()
            ...
            def MACD_bar(data):
                ...
                for i in range(0, len(data)):
                    value = data['MACD'][i] - data['signal'][i]
                ...
                for i in range(0, len(df)):
                    if df['MACD'][i] > df['signal'][i] :
                        ...
                        if flag == True:
                            ...
                            MACD_Sell.append(np.nan)
                    elif flag == True and df['Adj Close'][i] < MACD_Buy[-1] * (1 - risk):
                        MACD_Sell.append(df["Adj Close"][i])
                        MACD_Buy.append(np.nan)
                        flag = False
            ...
            #storing the function
            st.header("Buy and Sell startegy as per Moving Average Convergence and Divergence")
            st.subheader("Disclamer- This is just for educational purposes not an investment advice")
            ...
            fig.suptitle(selected_stocks, fontsize=10, backgroundcolor='blue', color='white')
            ...
            ax2.bar('Date', 'MACD_bar', data=data, label='Volume', color=data.positive.map({True: 'g', False: 'r'}), width=1,alpha=0.8)
            ...
            st.subheader("Disclamer- This is just for educational purposes not an investment advice")
            st.subheader("Bolinger Bands are not for Long term trends, they are suitable for short term purposes mostly!")
